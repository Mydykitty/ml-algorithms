...
from 机器学习.utils.hypothesis import sigmoid, sigmoid_gradient
...
class MultilayerPerceptron:
    ...
    def train(self, max_iterations=1000, alpha=0.1):
        ...
        (optimized_theta, cost_history) = MultilayerPerceptron.gradient_descend(self.data, self.labels, unrolled_theta,
    ...
    def thetas_init(layers):
        ...
        for layer_index in range(num_layers - 1):
            ...
            thetas[layer_index] = np.random.rand(out_count, in_count + 1) * 0.05  # 随机进行初始化操作
        return thetas
    ...
    def thetas_unroll(thetas):
        ...
        for theta_layer_index in range(num_theta_layers):
            flatten = thetas[theta_layer_index].flatten()
            unrolled_theta = np.hstack((unrolled_theta, flatten))
        return unrolled_theta

    @staticmethod
    def gradient_descend(data, labels, unrolled_theta, layers, max_iterations, alpha):
    ...
    def back_propagation(data, labels, thetas, layers):
        ...
        for example_index in range(num_examples):
            ...
            for layer_index in range(num_layers - 1):
                ...
                layers_activations[layer_index + 1] = layers_activation  # 后一层经过激活函数后的输出
